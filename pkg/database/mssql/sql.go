...
import (
	...
	"github.com/znyh/library/pkg/config/encoding"
)

const (
	_family          = "sqlserver_client"
...
func Open(c *Config) (*DB, error) {
	...
	query.Add("connection timeout", fmt.Sprintf("%d", c.ConnectionTimeout))
	if !c.EncryptEnabled {
		query.Add("encrypt", "disable")
	}
	query.Add("log", fmt.Sprintf("%d", c.LogFlags))
	if c.EncodeConfig {
		c.User, _ = encoding.Base64Decode(c.User)
		c.Password, _ = encoding.Base64Decode(c.Password)
	}
	u := &url.URL{
		Scheme: "sqlserver",
	...
	d, err := connect(c, u.String())
	if err != nil {
		return nil, err
	}
	addr := parseDSNAddr(c.Host)
	brkGroup := breaker.NewGroup(c.Breaker)
	brk := brkGroup.Get(addr)
	w := &conn{DB: d, breaker: brk, conf: c, addr: addr}
	rs := make([]*conn, 0, len(c.ReadHost))
	for _, rd := range c.ReadHost {
		u.Host = rd
		d, err := connect(c, u.String())
		if err != nil {
			return nil, err
...
func connect(c *Config, dataSourceName string) (*sql.DB, error) {
	d, err := sql.Open("sqlserver", dataSourceName)
	if err != nil {
